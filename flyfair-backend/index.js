    ...
    try {
        ...
        const flightPrices = response.data.map((flight) => {
            ...
            return {
                ...
                departTime: flight.itineraries[0].segments[0].departure.at,
                arriveTime: flight.itineraries[0].segments[0].arrival.at,
                departIATACode:
...
app.get('/city-suggestions', async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({ error: 'Failed to fetch city suggestions' });
    }
});
